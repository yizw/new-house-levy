...
import com.yizw.newhouselevy.AsyncTask.HouGYQYConfirmAsyncTask;
import com.yizw.newhouselevy.AsyncTask.HouseConfirmGYQYSaveAsyncTask;
import com.yizw.newhouselevy.Business.CONFIRM_GYQY_PRO_MON_Bus;
...
/**国有企业--房屋调查确认表（国有企业货币确认表和国有企业产权确认表一致）*/
public class HouGYQYConfirmTable extends OrmLiteBaseActivity<DatabaseHelper>{
	...
	public HouGYQYConfirmTable activity;
    ...
    private EditText  edit_houselocation_x4,edit_houseowner_x1,edit_licenseno, edit_authority, 
    edit_operatperiod,edit_busaddress, 
  //  edit_enterprisename_x1,
    edit_enterprisenature,
    edit_representative_x186,edit_tellink_x3,edit_paysecurity,edit_taxproof,
    edit_apprprocedure, edit_structure_x7_y,edit_area_x7_y, edit_structure_x8_y,
    edit_area_x8_y,edit_structure_x81_y,edit_area_x81_y,edit_structure_x84_y,
    edit_area_x84_y,edit_structure_x200_y,edit_area_x200_y,
    edit_structure_x201_y, edit_area_x201_y,
    edit_area_x11_y,
    edit_area_x7_n, edit_area_x8_n,
    edit_area_x81_n,edit_area_x84_n, edit_area_x200_n,
    edit_area_x201_n, 
    edit_area_x12_n,
    edit_x600,
    
//    edit_prono, edit_pronature,edit_prouse,edit_proarea,edit_approveno,edit_approvedepart,
...
//    edit_ismortgage,edit_isclose;
    ...
    edit_isoperat,edit_islegaluse;
    	
	
    private TextView text_structure_x7_n,text_structure_x8_n,text_structure_x81_n,
    text_structure_x84_n,text_structure_x200_n,text_structure_x201_n,text_maininquirer_x218,text_enterprisename_x1;
    ...
    AlertDialog statusDialog = null;
	 ...
	 public void onPause(){
		...
		protected void onCreate(Bundle savedInstanceState) {
			...
			setContentView(R.layout.hou_gyqy_pro_mon_confirmtable);	
			PublicBus.titleBarControl(this, "征收非住宅房屋调查确认表（国有）", true, null, true, null);
			initControl();
			operateIntent();
		}
	...
	private void operateIntent() {
		...
		if (StringHelper.isNullOrEmpty(in_id)) {
			...
			HouGYQYConfirmAsyncTask task = new HouGYQYConfirmAsyncTask(activity);
			task.execute(id);
			setEntityToUI(entity);
		}
	}
	...
	Spinner initSp(int id, String enmucode, boolean allowEmptyRow)
			throws MyException {
		Spinner sp = (Spinner) findViewById(id);
		PublicBus.spinnerEnumBinding(sp, enmucode, allowEmptyRow, getHelper(),
				this);
		return sp;
	}
		 
	/** 将国有企业房屋调查确认表的信息展现到UI */
	...
	class OnClick implements View.OnClickListener {
		...
		void Onclick_btn_confirmreg() {
			if(newactivity()){
			Intent intent = new Intent(activity, HouseConfirmRegListA.class);
			intent.putExtra(HouseConfirmRegListA.KEY.c_id, entity.getId());
		...
		boolean newactivity(){
			if(StringHelper.isNullOrEmpty(entity.getId())){
				Toast.makeText(activity, "确认表ID为空，请先保存(暂存或提交)确认表！", Toast.LENGTH_LONG).show();
				return false;
			}
			return true;
		}
...
//			try {
//				getEntity();
//				String result = CONFIRM_GYQY_PRO_MON_Bus.check(entity);
//				if (!result.equals("")) {
//					DialogHelper.showConfirm(activity, result);
//					return;
//				}
//				CONFIRM_GYQY_PRO_MON_Bus bus = new CONFIRM_GYQY_PRO_MON_Bus(
//						getHelper());
//				bus.create(entity);
//
...
//			});
			...
			builder.setPositiveButton("确认", new DialogInterface.OnClickListener() {
				public void onClick(DialogInterface dialog, int which) {
					// TODO Auto-generated method stub
					try { 
						HouseConfirmGYQYSaveAsyncTask save = new HouseConfirmGYQYSaveAsyncTask(activity);
						save.execute("");
					} catch (Exception e) {
		...
		void Onclick_btn_title_left() {
			if (isAdd == false) {
				HouGYQYConfirmTable.this.finish();
				return;
			}
			// 新增国有企业房屋调查确认表信息时，按下返回，如果必填项姓名或证件号不为空则进行关闭确认
			if (isAdd) {
				String houselocation = edit_houselocation_x4.getText().toString().trim();
				String houseowner = edit_houseowner_x1.getText().toString().trim();

				if ((!houselocation.equals("")) || (!houseowner.equals(""))) {
					new AlertDialog.Builder(HouGYQYConfirmTable.this)

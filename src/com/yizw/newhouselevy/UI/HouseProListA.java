...
import com.yizw.newhouselevy.Adapter.HouseProListAdapter;
import com.yizw.newhouselevy.AsyncTask.HouseProListAsyncTask;
...
public class HouseProListA extends OrmLiteBaseActivity<DatabaseHelper>{
	...
	public HouseProListA activity;
	public ListView listView;
	public TextView txt_list_footer;
	
    public String pid;
	
	public HouseProListAdapter adapter;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.house_pro_list);
		
        PublicBus.titleBarControl(this, "征收项目", true, null, true, "");     
  //    LoginBus.getLogin(this).setMainprojectnum(0);
	...
	private Button initBtn(int id){
	 ...
	 void initListPage(View footerView){
			...
			adapter = new HouseProListAdapter(activity, new DataTableList());//实例化一个空数据项的适配器
	 ...
	 /** 加载分页数据*/
		void loadListPageData(){
			Intent intent = this.getIntent();
  			...
  			HouseProListAsyncTask task = new HouseProListAsyncTask(activity);
			task.execute();
		}
		...
		class OnClick implements View.OnClickListener {
			...
			void Onclick_btn_title_right() {
				...
				adapter = new HouseProListAdapter(activity, new DataTableList());
				listView.setAdapter(adapter);	
				loadListPageData();
			}
		}
		...
		class OnItemClick_list implements OnItemClickListener{
				...
				public void onItemClick(AdapterView<?> parent, View view, int position,long id) {
					...
					if(loadedNum > position){
						...
						intent.putExtra(HouseNameListA.KEY.pid, projectid);

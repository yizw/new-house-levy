...
import com.yizw.newhouselevy.Adapter.HouseFloorListAdapter;
import com.yizw.newhouselevy.AsyncTask.HouseFloorListAsyncTask;
import com.yizw.newhouselevy.Business.PublicBus;
import com.yizw.newhouselevy.DAO.DatabaseHelper;
import com.yizw.newhouselevy.Entity.PageDomain;

/*房源--楼层列表*/
public class HouseFloorListA extends OrmLiteBaseActivity<DatabaseHelper>{
	
	public class KEY{	
		public static final String in_id = "in_id";
	}	

	public HouseFloorListA activity;
	public ListView listView;
	public TextView txt_list_footer;
	
   // public String pid;
	
	public HouseFloorListAdapter adapter;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        PublicBus.titleBarControl(this, "楼层列表", true, null, true, "");     
  //    LoginBus.getLogin(this).setMainprojectnum(0);
	...
	void initListPage(View footerView){
			...
			adapter = new HouseFloorListAdapter(activity, new DataTableList());//实例化一个空数据项的适配器
	 ...
	 /** 加载分页数据*/
		void loadListPageData(){
  		...
  		//	pid = intent.getStringExtra(KEY.in_id); 
			HouseFloorListAsyncTask task = new HouseFloorListAsyncTask(activity);
			task.execute();
		}
		...
		class OnClick implements View.OnClickListener {
			...
			void Onclick_btn_title_right() {
				...
				adapter = new HouseFloorListAdapter(activity, new DataTableList());
				listView.setAdapter(adapter);	
				loadListPageData();
			}
		}
		...
		class OnItemClick_list implements OnItemClickListener{
				...
				public void onItemClick(AdapterView<?> parent, View view, int position,long id) {
					...
					if(loadedNum > position){
						...
						String housefloorid = txt_id.getText().toString();	
												
						Intent intent = new Intent(activity,HouseRoomListA.class);						
						intent.putExtra(HouseRoomListA.KEY.in_id, housefloorid);

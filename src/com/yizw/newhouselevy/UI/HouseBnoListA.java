...
import com.yizw.newhouselevy.Adapter.HouseBnoListAdapter;
import com.yizw.newhouselevy.AsyncTask.HouseBnoListAsyncTask;
import com.yizw.newhouselevy.Business.PublicBus;
import com.yizw.newhouselevy.DAO.DatabaseHelper;
import com.yizw.newhouselevy.Entity.PageDomain;

/*房源--楼栋列表*/
public class HouseBnoListA extends OrmLiteBaseActivity<DatabaseHelper>{
	
	public class KEY{	
		public static final String in_id = "in_id";
	}	

	public HouseBnoListA activity;
	public ListView listView;
	public TextView txt_list_footer;
	
   // public String pid;
	
	public HouseBnoListAdapter adapter;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.house_bno_list);
		
        PublicBus.titleBarControl(this, "楼栋列表", true, null, true, "");     
  //    LoginBus.getLogin(this).setMainprojectnum(0);
	...
	private Button initBtn(int id){
	 ...
	 void initListPage(View footerView){
			...
			adapter = new HouseBnoListAdapter(activity, new DataTableList());//实例化一个空数据项的适配器
	 ...
	 /** 加载分页数据*/
		...
		//	Intent intent = this.getIntent();
  		...
  		//	pid = intent.getStringExtra(KEY.in_id); 
			HouseBnoListAsyncTask task = new HouseBnoListAsyncTask(activity);
			task.execute();
		}
		...
		class OnClick implements View.OnClickListener {
			...
			void Onclick_btn_title_right() {
				...
				adapter = new HouseBnoListAdapter(activity, new DataTableList());
				listView.setAdapter(adapter);	
				loadListPageData();
			}
		}
		...
		class OnItemClick_list implements OnItemClickListener{
				...
				public void onItemClick(AdapterView<?> parent, View view, int position,long id) {
					...
					if(loadedNum > position){
						...
						String housebnoid = txt_id.getText().toString();	
												
						Intent intent = new Intent(activity,HouseUnitListA.class);						
						intent.putExtra(HouseUnitListA.KEY.in_id, housebnoid);

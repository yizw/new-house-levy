...
import com.yizw.newhouselevy.AsyncTask.GYGRFamilyRegSaveAsyncTask;
...
/*国有个人房屋调查确认表选择户籍人口*/
public class GYGRConfirmPerson extends OrmLiteBaseActivity<DatabaseHelper>{
	//页面间数据传输用到的key
	public class KEY{
		/** 确认表户籍人员索引*/
		public static final String GYGRconfirmPersonIndex = "GYGRconfirmPersonIndex";
	...
	public GYGRConfirmPerson activity;
...
//	  String cstatus = intent.getStringExtra(c_status);
//	  c_status = cstatus;
	  
		
	  entity = (CONFIRM_PERSON)intent.getSerializableExtra(KEY.GYGRconfirmPersonIndex);
	...
	class OnClick implements OnClickListener{
		...
		void Onclick_btn_title_right() {
			getEntity();
			String result = CONFIRM_GYGR_PRO_MON_Bus.checkPerson(entity);
			if(!result.equals("")){
				DialogHelper.showConfirm(activity,result);
				return;
			}
			GYGRFamilyRegSaveAsyncTask task = new GYGRFamilyRegSaveAsyncTask(activity);
			task.execute("");
		}

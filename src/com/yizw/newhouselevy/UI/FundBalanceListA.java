...
import com.yizw.newhouselevy.Adapter.FundBalanceListAdapter;
import com.yizw.newhouselevy.AsyncTask.FundBalanceListAsyncTask;
import com.yizw.newhouselevy.Business.PublicBus;
import com.yizw.newhouselevy.DAO.DatabaseHelper;
import com.yizw.newhouselevy.Entity.PageDomain;

/**各征收项目列表**/
public class FundBalanceListA extends OrmLiteBaseActivity<DatabaseHelper>{
	
	public class KEY{	
		public static final String in_id = "in_id";
	}	

	public FundBalanceListA activity;
	public ListView listView;
	public TextView txt_list_footer;
	
    public String pid;
	
	public FundBalanceListAdapter adapter;
  ...
  //    LoginBus.getLogin(this).setMainprojectnum(0);
	...
	private Button initBtn(int id){
	 ...
	 void initListPage(View footerView){
			...
			adapter = new FundBalanceListAdapter(activity, new DataTableList());//实例化一个空数据项的适配器
	 ...
	 /** 加载分页数据*/
		void loadListPageData(){
			Intent intent = this.getIntent();
  			...
  			FundBalanceListAsyncTask task = new FundBalanceListAsyncTask(activity);
			task.execute();
		}
		...
		class OnClick implements View.OnClickListener {
			...
			void Onclick_btn_title_right() {
				...
				adapter = new FundBalanceListAdapter(activity, new DataTableList());
				listView.setAdapter(adapter);	
				loadListPageData();
			}
		}
		...
		class OnItemClick_list implements OnItemClickListener{
				...
				public void onItemClick(AdapterView<?> parent, View view, int position,long id) {
					...
					if(loadedNum > position){
						...
						intent.putExtra(FundBalanceInfoA.KEY.in_id, projectid);
						intent.putExtra(FundBalanceInfoA.KEY.pid, pid);

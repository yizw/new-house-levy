...
import com.yizw.newhouselevy.Adapter.ProProblemFeedbackListAdapter;
import com.yizw.newhouselevy.AsyncTask.ProListAsyncTask;
...
/*项目 问题反馈的项目列表*/
public class ProProblemFeedbackListA extends OrmLiteBaseActivity<DatabaseHelper>{
	...
	public ProProblemFeedbackListA activity;
	public ListView listView;
	public TextView txt_list_footer;
	
    public String pid;
	
	public ProProblemFeedbackListAdapter adapter;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        PublicBus.titleBarControl(this, "项目列表", true, null, true, "");     
  //    LoginBus.getLogin(this).setMainprojectnum(0);
	...
	private Button initBtn(int id){
	 ...
	 void initListPage(View footerView){
			...
			adapter = new ProProblemFeedbackListAdapter(activity, new DataTableList());//实例化一个空数据项的适配器
	 ...
	 /** 加载分页数据*/
		void loadListPageData(){
			Intent intent = this.getIntent();
  			...
  			ProListAsyncTask task = new ProListAsyncTask(activity);
			task.execute();
		}
		...
		class OnClick implements View.OnClickListener {
			...
			void Onclick_btn_title_right() {
				...
				adapter = new ProProblemFeedbackListAdapter(activity, new DataTableList());
				listView.setAdapter(adapter);	
				loadListPageData();
			}
		}
		...
		class OnItemClick_list implements OnItemClickListener{
				...
				public void onItemClick(AdapterView<?> parent, View view, int position,long id) {
					...
					if(loadedNum > position){
						...
						intent.putExtra(ProQuestionListA.KEY.in_id, projectid);
						intent.putExtra(ProQuestionListA.KEY.pid, projectid);

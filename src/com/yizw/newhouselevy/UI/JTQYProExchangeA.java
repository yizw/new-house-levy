...
import com.yizw.newhouselevy.AsyncTask.JTQYProAsyncTask;
import com.yizw.newhouselevy.AsyncTask.JTQYProSaveAsyncTask;
...
/**分户--集体企业--产权--补偿计算单*/
public class JTQYProExchangeA extends OrmLiteBaseTabActivity<DatabaseHelper>{
	...
	public JTQYProExchangeA activity;
	
	public HouseList entity;
	
	public HouseConfirm c;
	
//	public HouseList jtqyproexchange;
    ...
    edit_value_x22,    
    //tab3
    edit_value_x24,edit_cost_x187, edit_money_x39,edit_money_x168,edit_money_x169,
  //3月18日新需求，国有企业产权、国有企业货币、集体企业产权、集体企业货币，X107、X108、X109调整为手动录入
    ...
    edit_value_x58,edit_area_x66, edit_value_x67,edit_area_x230,edit_value_x213,
    edit_area_x215,edit_value_x216,
  ...
  //text_cardno_x2,
   ...
   // text_cost_x107,text_cost_x108, text_cost_x109,
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.hou_jtqy_proexchange);
		InitTabHost();
		initControl();
		operateIntent();
	}
	...
	inflater.inflate(R.layout.hou_jtqy_proexchange_tab1, tabHost.getTabContentView());
	tabHost.addTab(tabHost.newTabSpec("tab1").setIndicator(getTabItemView(inflater,0,"一")).setContent(R.id.hou_jtqy_proexchange_tab1));
	tabHost.getTabWidget().getChildAt(0).setBackgroundResource(R.drawable.tab_item_bg);
		 
	inflater.inflate(R.layout.hou_jtqy_proexchange_tab2, tabHost.getTabContentView());
	tabHost.addTab(tabHost.newTabSpec("tab2").setIndicator(getTabItemView(inflater,0,"二")).setContent(R.id.hou_jtqy_proexchange_tab2));
	tabHost.getTabWidget().getChildAt(1).setBackgroundResource(R.drawable.tab_item_bg);
		 
	inflater.inflate(R.layout.hou_jtqy_proexchange_tab3, tabHost.getTabContentView());
	tabHost.addTab(tabHost.newTabSpec("tab3").setIndicator(getTabItemView(inflater,0,"三")).setContent(R.id.hou_jtqy_proexchange_tab3));
	tabHost.getTabWidget().getChildAt(2).setBackgroundResource(R.drawable.tab_item_bg);
		     
	inflater.inflate(R.layout.hou_jtqy_proexchange_tab4, tabHost.getTabContentView());
	tabHost.addTab(tabHost.newTabSpec("tab4").setIndicator(getTabItemView(inflater,0,"四")).setContent(R.id.hou_jtqy_proexchange_tab4));
	...
	//	edit_linktel_x3 = (EditText) findViewById(R.id.edit_linktel_x3);
		...
		edit_area_x230 = (EditText) findViewById(R.id.edit_area_x230);
		edit_value_x213 = (EditText) findViewById(R.id.edit_value_x213);
		edit_area_x215 = (EditText) findViewById(R.id.edit_area_x215);
...
//			JTQYProAsyncTask task = new JTQYProAsyncTask(activity);
//			task.execute(in_id); 
	...
	/** 将集体企业 产权调换补偿计算单的信息展现到UI */
//	public void setEntityToUI(HouseList jtqyproexchange,HouseConfirm c){
	public void setEntityToUI(HouseList entity,HouseConfirm c){	
		
		DecimalFormat DouToStr = new DecimalFormat("0.00");
		
		// tab1		
	//	jtqyproexchange.setHouseConfirm(c);
	...
	//	edit_linktel_x3.setText(c.getX3());
		...
		//tab4
		edit_area_x53.setText(DouToStr.format(entity.getX152()));
		edit_value_x56.setText(DouToStr.format(entity.getX153()));
		edit_area_x54.setText(DouToStr.format(entity.getX155()));
		edit_value_x57.setText(DouToStr.format(entity.getX156()));
		edit_area_x55.setText(DouToStr.format(entity.getX158()));
		edit_value_x58.setText(DouToStr.format(entity.getX159()));
		edit_area_x66.setText(DouToStr.format(entity.getX212()));
		edit_value_x67.setText(DouToStr.format(entity.getX215()));
		edit_area_x230.setText(DouToStr.format(entity.getX213()));
		edit_value_x213.setText(DouToStr.format(entity.getX216()));
		edit_area_x215.setText(DouToStr.format(entity.getX214()));
...
//		text_cardno_x2.setText(entity.getX2());
	...
	//  entity.setX3(edit_linktel_x3.getText().toString());
		...
		entity.setX213(StringToDouble(edit_area_x230.getText().toString()));
		entity.setX216(StringToDouble(edit_value_x213.getText().toString()));
		entity.setX214(StringToDouble(edit_area_x215.getText().toString()));
	...
	class OnClick implements View.OnClickListener {
		...
		void showTempSaveDialog(String message) {
			...
			builder.setTitle("集体企业产权算单提交或暂存");
				...
				builder.setSingleChoiceItems(new String[] {"暂时保存"}, 0, new DialogInterface.OnClickListener() {  
					...
					builder.setPositiveButton("确认", new DialogInterface.OnClickListener() {
						public void onClick(DialogInterface dialog, int which) {
							// TODO Auto-generated method stub
							try { 
								JTQYProSaveAsyncTask save = new JTQYProSaveAsyncTask(activity);
								save.execute("");
							} catch (Exception e) {
		...
		void Onclick_btn_title_left() {
			if (isAdd == false) {
				JTQYProExchangeA.this.finish();
				return;
			}
			// 新增集体企业 产权调换补偿计算单信息时，按下返回，如果必填项姓名或证件号不为空则进行关闭确认
			if (isAdd) {
				...
				if ((!levyname.equals("")) || (!cardno.equals(""))) {
					new AlertDialog.Builder(JTQYProExchangeA.this)

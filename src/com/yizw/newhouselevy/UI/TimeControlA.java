...
import com.yizw.newhouselevy.Adapter.TimeControlListAdapter;
import com.yizw.newhouselevy.AsyncTask.TimeControlAsyncTask;
import com.yizw.newhouselevy.Business.PublicBus;
import com.yizw.newhouselevy.DAO.DatabaseHelper;
import com.yizw.newhouselevy.Entity.PageDomain;

/**时限控制项目列表**/
public class TimeControlA extends OrmLiteBaseActivity<DatabaseHelper> {
	
	public TimeControlA activity;
	public ListView listView;
	public TextView txt_list_footer;
	
	public TimeControlListAdapter adapter;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.time_control_list);
        PublicBus.titleBarControl(this, "时限控制", true, null, true, "");     
        initControl();
        loadListPageData();
    }	
    ...
    void initListPage(View footerView){
		...
		adapter = new TimeControlListAdapter(activity, new DataTableList());//实例化一个空数据项的适配器
	...
	void loadListPageData(){
		TimeControlAsyncTask task = new TimeControlAsyncTask(activity);
		task.execute();
	}
	...
	class OnClick implements View.OnClickListener {
		...
		void Onclick_btn_title_right() {
			...
			adapter = new TimeControlListAdapter(activity, new DataTableList());
			listView.setAdapter(adapter);	
			loadListPageData();
		}
	}
	...
	class OnItemClick_list implements OnItemClickListener{
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,long id) {
			...
			if(loadedNum > position){
				...
				String ggid = txt_id.getText().toString();
				TextView txt_project_type = (TextView) view.findViewById(R.id.project_type);
				String project_type = txt_project_type.getText().toString();
				
				Intent intent = new Intent();
				intent.setClass(TimeControlA.this, TimeControlInfoA.class);
				intent.putExtra(TimeControlInfoA.KEY.in_id, ggid);
				intent.putExtra(TimeControlInfoA.KEY.project_type, project_type);

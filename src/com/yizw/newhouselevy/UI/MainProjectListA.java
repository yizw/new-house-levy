...
import com.yizw.newhouselevy.Adapter.MainProjectListAdapter;
import com.yizw.newhouselevy.AsyncTask.MainProjectListPageAsyncTask;
...
/**征收项目总览列表**/
public class MainProjectListA extends OrmLiteBaseActivity<DatabaseHelper> {
	...
	public MainProjectListA activity;
	public ListView listView;
	public TextView txt_list_footer;
	
	public MainProjectListAdapter adapter;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.main_projectlist);
		
        PublicBus.titleBarControl(this, "项目总览", true, null, true, "");     
    ...
    void initListPage(View footerView){
		...
		adapter = new MainProjectListAdapter(activity, new DataTableList());//实例化一个空数据项的适配器
	...
	void loadListPageData(){
		...
		MainProjectListPageAsyncTask task = new MainProjectListPageAsyncTask(activity);
		task.execute();
	}
	...
	class OnClick implements View.OnClickListener {
		...
		void Onclick_btn_title_right() {
			...
			adapter = new MainProjectListAdapter(activity, new DataTableList());
			listView.setAdapter(adapter);	
			loadListPageData();
		}
	}
...
//				String pid = txt_id.getText().toString();
				
				TextView txt_id = (TextView) view.findViewById(R.id.txt_id);
				String projectid = txt_id.getText().toString();	
				
				Intent intent = new Intent(activity,MainProjectIndexA.class);						
				intent.putExtra(MainProjectIndexA.KEY.in_id, projectid);
				intent.putExtra(MainProjectIndexA.KEY.pid, projectid);

...
import com.yizw.newhouselevy.Adapter.DataBaseListAdapter;
import com.yizw.newhouselevy.AsyncTask.DataBaseListPageAsyncTask;
...
/* 资料库列表 */
public class DataBaseListA extends OrmLiteBaseActivity<DatabaseHelper>{
	
	public DataBaseListA activity;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        PublicBus.titleBarControl(this, "资料库信息列表", true, null, true, null);     
        initControl();
        loadListPageData();
    }	
	...
	void initListPage(View footerView){
		...
		adapter = new DataBaseListAdapter(activity, new DataTableList());//实例化一个空数据项的适配器
	...
	void loadListPageData(){
		DataBaseListPageAsyncTask task = new DataBaseListPageAsyncTask(activity);
		task.execute();
	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if(resultCode==RESULT_OK){
			...
			adapter = new DataBaseListAdapter(activity, new DataTableList());
			listView.setAdapter(adapter);	
			loadListPageData();
		}
	}
	...
	class OnItemClick_list implements OnItemClickListener{
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,long id) {
			...
			if(loadedNum > position){
				...
				String dbid = txt_id.getText().toString();
				
				Intent intent = new Intent(activity, DataBaseInfoA.class);
				intent.putExtra(DataBaseInfoA.KEY.in_id, dbid);

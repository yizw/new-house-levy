...
import com.yizw.newhouselevy.Adapter.ProReplyAdapter;
import com.yizw.newhouselevy.AsyncTask.ProReplySearchAsyncTask;
import com.yizw.newhouselevy.Business.PublicBus;
import com.yizw.newhouselevy.DAO.DatabaseHelper;
import com.yizw.newhouselevy.Entity.PageDomain;

/** 问题反馈(项目) 反馈信息*/
public class ProReplyListA extends OrmLiteBaseActivity<DatabaseHelper>{
  public class KEY{	
		/** id */
	...
	public ProReplyAdapter adapter;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.problem_feedback_reply_list);
		...
		void initListPage(View footerView){
				...
				adapter = new ProReplyAdapter(activity, new DataTableList());//实例化一个空数据项的适配器
  ...
  void loadListPageData() {
	...
	ProReplySearchAsyncTask task = new ProReplySearchAsyncTask(activity);
	task.execute();
	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (resultCode == RESULT_OK) {
			...
			adapter = new ProReplyAdapter(activity,new DataTableList());
			listView.setAdapter(adapter);
			loadListPageData();
		}
	}
...
//				}
			...
			class OnClick implements View.OnClickListener {
				...
				void Onclick_btn_title_right() {
					...
					intent.putExtra(ProReplyAdd.KEY.question_id, in_id);

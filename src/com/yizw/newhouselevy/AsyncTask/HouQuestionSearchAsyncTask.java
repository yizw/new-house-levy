...
import com.yizw.newhouselevy.UI.HouQuestionListA;

/*问题反馈（分户） 问题列表查询*/
public class HouQuestionSearchAsyncTask extends AsyncTask<Void, String, HashMap<String, DataTableList>> {

	private HouQuestionListA activity;

	public HouQuestionSearchAsyncTask(HouQuestionListA activity) {
	...
	protected HashMap<String, DataTableList> doInBackground(Void... params) {
		...
		try {
			...
			UploadCmd cmd = new UploadCmd(CmdCode.SEARCH_PROFEEDBACK_QUESTION_INFO);
			cmd.addParameter("hid", activity.household_id);
	...
	protected void onPostExecute(HashMap<String, DataTableList> result) {
		if (result == null) {
			...
			activity.pageDomain.itemTotal = Integer.valueOf(pageList.get(0)
					.get("total").toString());
			activity.pageDomain.pageItems = Integer.valueOf(pageList.get(0)

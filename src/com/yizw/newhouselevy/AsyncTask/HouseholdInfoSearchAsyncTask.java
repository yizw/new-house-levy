...
public class HouseholdInfoSearchAsyncTask extends AsyncTask<String, String, DataTableList> {

	private HouseholdInfoA activity;
	AlertDialog statusDialog;
	
	public HouseholdInfoSearchAsyncTask(HouseholdInfoA activity){
	...
	protected DataTableList doInBackground(String... params) {
		...
		try {
			...
			UploadCmd cmd = new UploadCmd(CmdCode.SEARCH_PRO_HOUSEHOLD_INFO);
	...
	protected void onPostExecute(DataTableList result) {
		//关掉状态提示
				...
				try {
					MsgHelper.setValuesToEntity(activity.household, result);
					activity.setEntityToUI(activity.household);
				} catch (MyException e) {

...
import com.yizw.newhouselevy.UI.HouseholdPushTableA;

public class HouseholdPushTableAsyncTask extends AsyncTask<Void, String, HashMap<String, DataTableList>> {

	private HouseholdPushTableA activity;
	
	public HouseholdPushTableAsyncTask(HouseholdPushTableA activity){
	...
	protected HashMap<String, DataTableList> doInBackground(Void...params) {
		...
		try {
			...
			UploadCmd cmd = new UploadCmd(CmdCode.SEARCH_PROGRESS_BRANCHLIST);
			cmd.addParameter("householdid", activity.householdid); 
			msg.addCmd(cmd);
